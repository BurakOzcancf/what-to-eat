...
const FoodsDetails = () => {
  ...
  React.useEffect(() => {
    axios
      .get(
        `${baseURL}${params.det}/information?apiKey=${process.env.REACT_APP_API}`
      )
      .then((response) => {
  ...
  return (
    <div>
      <button
        className="bg-slate-500 text-white py-2 px-4 rounded-xl ml-5"
        onClick={() => navigate("/")}
      >
        Back
      </button>
        ...
        <div className="p-10 max-w-5xl m-auto">
          ...
          <ul className="list-disc py-3">
            ...
            {food.extendedIngredients.map((item, index) => (
              <li key={index}>
                {item.name} {item.measures.metric.amount}{" "}
